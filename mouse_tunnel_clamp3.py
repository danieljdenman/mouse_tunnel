...
## Updates EAS: Line 701 for changing duration required between image off and first lick: gradually build down to < 1 sec
...
class MouseTunnel(ShowBase):
    def __init__(self):
        ...
        #self.stim_duration= 2.0 # in seconds  #EAS
        #self.max_stim_duration = 3.0 # in seconds #EAS

        self.stim_duration= 2.5 # in seconds  #EAS
        self.max_stim_duration = 3.5 # in seconds #EAS
        ...
        self.reward_time = 0.3 # in sec, based on volume. hard coded right now but should be modified by the (1) calibration and (2) optionally by the main loop for dynamic reward scheduling
...
#        return int(score / 20.)
    ...
    def _give_manual_reward(self,volume):
        self.manualRewardData.extend([globalClock.getFrameTime()])
        self._give_reward(volume)
    ...
    def _toggle_stop_reward(self):
        self.update_mode()
        if self.STOP_REWARD:
            self.STOP_REWARD = False
            print('switched to NOT rewarding for all stops, only for current trial tiles')
        else:
            self.STOP_REWARD = True
            print('switched to rewarding for all stops. at '+str(self.stop_reward_fraction) + 'of typical reward volume: '+str(REWARD_VOLUME))

    def update_mode(self):
        state = list(self.modestring)
        if self.LICK_ABORT: state[0] = 'K' 
        else: state[0]='k'
        if self.AUTO_REWARD: state[1]='L'
        else: state[1]='l'
        if self.STOP_REWARD: state[2]='S'
        else: state[2]='s'
        self.modestring = "".join(state)
        self.modestringData[globalClock.getFrameTime()] = self.modestring
...
#                    if len(np.where(np.array(self.lickData) > self.stim_off_time)[0]) > 1: # this checks if there has been more than zero licks since the stimulus turned off
                    ...
                    #EAS gradually build them down to 1 sec allowed between stim off & first lick:
                    if len(np.where(np.array(self.lickData) > self.stim_off_time)[0]) > 5: # this checks if there has been more than zero licks since the stimulus turned off
    ...
    def close(self):
        ...
        np.save(os.path.join(save_path,'manualRewardData.npy'),self.manualRewardData)
        np.save(os.path.join(save_path,'modestringData.npy'),self.modestringData)
